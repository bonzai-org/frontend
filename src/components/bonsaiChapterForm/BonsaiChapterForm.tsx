...
const BonsaiChapterForm = () => {
    ...
    const handlePhotoChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        if (event.target.files && event.target.files.length > 0) {
            setBonsaiChapter({ ...bonsaiChapter, photos: [...bonsaiChapter.photos, event.target.files[0]] });
        }
    };
    ...
    const Photo = ({ photo, index }: { photo: File | null, index: number }) => {
        ...
        return (
            <div ref={(node) => ref(drop(node))} style={{ display: 'flex', alignItems: 'center', position: 'relative' }}>
                <img src={URL.createObjectURL(photo!)} alt={`Preview ${index}`} style={{ maxWidth: '200px', maxHeight: '200px' }} />
                <button 
                    type="button" 
        ...
        <DndProvider backend={HTML5Backend}>
            <form onSubmit={handleSubmit}>
                ...
                <div>
                    ...
                    <input type="text" id="caption" value={bonsaiChapter.caption} onChange={handleCaptionChange} />
                </div>
                <div>
                ...
                <div>
                    {bonsaiChapter.photos.length > 0 && (
                        <div>
                            <h3>Photo Preview:</h3>
                            {bonsaiChapter.photos.map((photo, index) => (
